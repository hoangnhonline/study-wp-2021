...
final class Plugin {
	...
	public function admin_enqueue_scripts() {
		...
		if ( WP_TRAVEL_ENGINE_POST_TYPE === $current_screen->id ) {
		...
		wp_register_style( $this->plugin_name . '_core_ui', plugin_dir_url( WP_TRAVEL_ENGINE_FILE_PATH ) . "dist/admin/wte-admin{$suffix}.css", array( 'wte-select2', 'wte-global', 'datepicker-style', 'animate' ), $this->version );
	...
	public function enqueue_scripts() {
		...
		$l10n = array(
			...
			'extensions'         => apply_filters( 'wte_active_extensions', array() ),
			'locale'             => get_locale(),
			'l10n'               => [
				'invalidCartTraveler' => __( 'No. of Travellers\' should be at least 1', 'wp-travel-engine' )
			]
		);
		...
		wp_register_script(
			...
			array( 'wte-global', 'wte-fpickr', 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'jquery-steps', 'jquery-validate', 'wte-fontawesome-all', 'v4-shims', 'toastr', 'jquery-sticky-kit', 'parsley', 'magnific-popup', 'owl-carousel', 'wte-rrule', 'moment', 'wte-moment-tz' ),
			$this->version,
			! 0
		);
