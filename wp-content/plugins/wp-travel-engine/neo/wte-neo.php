...
add_action(
	...
	function() {
		...
		// Migrate Trip.
		if ( isset( $_REQUEST['wte-action'] ) && 'trip-migration-500' === $_REQUEST['wte-action'] && isset( $_REQUEST['trip'] ) ) { // phpcs:ignore
			$trip = get_post( +$_REQUEST['trip'] );
			if ( ! ! $trip && ! function_exists( 'WTE\Upgrade500\migrate_trip_dates_and_pricings' ) ) {
				include_once sprintf( '%s/upgrade/500.php', WP_TRAVEL_ENGINE_BASE_PATH );
				WTE\Upgrade500\migrate_trip_dates_and_pricings( $trip );
				wp_redirect(
					add_query_arg(
						array(
							'post'   => $_REQUEST['trip'],
			...
			array(
				...
				'package-categories'       => array(
					...
					'get_callback' => function( $prepared, $field ) {
						...
						$categories = $wpdb->get_results( "SELECT {$wpdb->terms}.term_id, {$wpdb->terms}.name FROM {$wpdb->term_taxonomy} INNER JOIN {$wpdb->terms} ON {$wpdb->term_taxonomy}.term_id = {$wpdb->terms}.term_id WHERE taxonomy = 'trip-packages-categories'" );
...
function wte_update_trip_packages( $post_ID, $posted_data ) {
	...
	$packages_titles            = isset( $posted_data['packages_titles'] ) ? $posted_data['packages_titles'] : array();
	...
	update_post_meta( $post_ID, 'wp_travel_engine_setting_trip_actual_price', $lowest_price );
